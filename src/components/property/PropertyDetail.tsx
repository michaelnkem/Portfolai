    ...
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="w-full max-w-3xl bg-panel border-l border-border overflow-y-auto flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-panel border-b border-border p-5 flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap mt-1">
              {p?.postcode && <span className="text-xs font-mono bg-accent/10 border border-accent/20 text-accent px-2 py-0.5 rounded">{String(p.postcode)}</span>}
              {p?.property_type && <span className="text-xs text-mid">{String(p.property_type)}</span>}
              {p?.bedrooms && <span className="text-xs text-mid">· {String(p.bedrooms)} bed</span>}
              {p?.tenure && <span className="text-xs text-mid">· {String(p.tenure)}</span>}
              {cityName && <span className="text-xs text-mid">· {String(cityName)}</span>}
            </div>
          </div>
        ...
        <div className="p-5 grid grid-cols-2 sm:grid-cols-5 gap-3 border-b border-border">
          <Stat label="EST. CURRENT VALUE" value={estimatedCurrentValue ? `£${estimatedCurrentValue.toLocaleString()}` : 'No record'} tone="green"
          ...
          <Stat label="GROSS YIELD" value={grossYield ? `${grossYield}%` : 'Set rent →'} tone={grossYield > 6 ? 'green' : 'neutral'}
          ...
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-4 py-2.5 text-xs rounded-t-lg border-b-2 transition-all ${
                tab === t.id ? 'border-accent text-accent bg-accent/5' : 'border-transparent text-mid hover:text-white'
              }`}>{t.label}</button>
          ))}
        </div>
          ...
          {tab === 'overview' && (
            <div className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                {/* Property attributes with EST badges */}
                  ...
                  {(() => {
                    ...
                    const rows: Row[] = [
                      ...
                      ['EPC Rating',  epcRating !== '?' ? epcRating : 'Unknown',
                        epcRating !== '?' && epcRating !== 'Unknown',                                          false],
                      ...
                      ['Council Tax', p?.council_tax_band ? `Band ${p.council_tax_band}` : 'Unknown',  false, false],
                    ]
                    ...
                    return (
                      <>
                        {rows.map(([k, v, isEpc, isInferred]) => (
                          <div key={k} className="flex justify-between items-center py-2 border-b border-border text-sm">
                            ...
                            <span className="flex items-center gap-1.5">
                              {isInferred && (
                                <span className="text-[9px] font-mono text-dim border border-border/60 rounded px-1 py-0.5 leading-none">EST</span>
                              )}
                        ...
                        {(bedsInferred || tenureInferred || gardenInferred || bathsInferred) && (
                          <p className="text-[10px] text-dim mt-3">
                            <span className="font-mono border border-border/60 rounded px-1 mr-1">EST</span>
                            Estimated from floor area and property norms. Verify with agent.
                          </p>
                        )}
                      </>
                    )
                ...
                {cityData && (
                  <div className="space-y-4">
                    ...
                    <CityMarketPanel
                      cityName={cityName} cityData={cityData} propertyPrice={price}
                      estimatedCurrentValue={estimatedCurrentValue} propertyGrossYield={grossYield}
                      propertyNetYield={netYield} propertyRent={effectiveRent} propertyBeds={Number(p?.bedrooms ?? 2)}
                    />
                  </div>
                )}
              </div>
            ...
            <div className="space-y-5">
                  ...
                  <div className="flex gap-2">
                    {[defaultServiceCharge(p), 0].map(v => v).filter(Boolean).map(v => (
                      <button key={v} onClick={() => { setServiceCharge(v); setGroundRent(p?.tenure === 'Leasehold' ? 200 : 0) }}
                        className="btn-ghost text-xs py-2 px-3">Set defaults</button>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="card p-4 space-y-3">
                  ...
                  {[
                    ...
                    <div key={f.label}>
                      <div className="flex justify-between text-xs mb-1"><span className="text-mid">{f.label}</span><span className="text-white">£{f.val}</span></div>
                      <input type="range" min={f.min} max={f.max} step={f.step} value={f.val} onChange={e => f.set(Number(e.target.value))} className="w-full accent-accent" />
                    </div>
                  ))}
                  {[
                    ...
                    <div key={f.label}>
                      <div className="flex justify-between text-xs mb-1"><span className="text-mid">{f.label}</span></div>
                      <input type="range" min={f.min} max={f.max} step={f.step} value={f.val} onChange={e => f.set(Number(e.target.value))} className="w-full accent-accent" />
                    </div>
                  ))}
                </div>
                ...
                <div className="card p-4">
                  ...
                  {[
                    ...
                    <div key={row.k} className={`flex justify-between py-2 border-b border-border text-sm ${row.bold ? 'font-bold' : ''}`}>
                      ...
                      <span className={row.bold ? 'text-gold' : row.green ? 'text-accent' : row.red ? 'text-danger' : 'text-white'}>{row.v}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="card p-5">
                ...
                <div className="grid grid-cols-4 gap-3">
                  ...
                  <Stat label="CASHFLOW" value={`£${cashflow.toLocaleString()}/mo`} tone={cashflow >= 0 ? 'green' : 'red'} sub={cashflow >= 0 ? 'Positive ✓' : 'Negative ⚠'} />
                </div>
            ...
            <div className="space-y-4">
              <div className={`card p-5 ${!epcCompliant ? 'border-danger/30' : 'border-accent/30'}`}>
                ...
                <div className="flex items-center gap-4">
                  <div className={`text-5xl font-display font-black ${epcRating <= 'B' ? 'text-accent' : epcRating <= 'D' ? 'text-gold' : 'text-danger'}`}>{epcRating}</div>
                  <div>
                    ...
                    {risks.map((r, i) => {
                      ...
                      return (
                        <div key={i} className="flex justify-between items-center py-2 border-b border-border text-sm">
                          ...
                          <span className={`font-semibold ${tone}`}>{String(r.label ?? '')}</span>
                        </div>
                      )
                    })}
                  </div>
                ...
                <>
                  <div className="card p-5">
                    ...
                    <LineChart
                      ...
                      labels={projectionData.map(y => String(y.year))} height={130} />
...
function CityMarketPanel({
  ...
  cityName: string; cityData: Record<string, number>; propertyPrice: number
  estimatedCurrentValue: number; propertyGrossYield: number; propertyNetYield: number
  propertyRent: number; propertyBeds: number
}) {
  ...
  const BEDS = [
    { key: 'studio', label: 'Studio' }, { key: '1bed', label: '1 Bed' }, { key: '2bed', label: '2 Bed' },
    { key: '3bed', label: '3 Bed' }, { key: '4bed', label: '4 Bed' },
  ]
  ...
  const diff = (prop: number, city: number, higherIsBetter = true) => {
    ...
    return <span className={`text-[10px] ml-1.5 font-mono ${better ? 'text-accent' : 'text-danger'}`}>{d > 0 ? '+' : ''}{d.toFixed(1)}{city < 100 ? '%' : ''}</span>
  }
    ...
    <div className="card p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <select value={selectedCity} onChange={e => setSelectedCity(e.target.value)}
          ...
          {cities.map(c => <option key={c} value={c}>{c}{c === cityName ? ' ★' : ''}</option>)}
        </select>
      </div>
      <div className="flex gap-1.5 mb-4 flex-wrap items-center">
        {BEDS.map(b => (
          <button key={b.key} onClick={() => setSelectedBed(b.key)}
            className={`px-3 py-1 rounded-lg text-[11px] font-mono transition-all ${selectedBed === b.key ? 'bg-accent/20 border border-accent/40 text-accent' : 'bg-white/[0.03] border border-border text-dim hover:text-mid'}`}>
            {b.label}{b.key === defaultBedKey && <span className="ml-1 text-[9px] text-accent/50">★</span>}
          </button>
        ))}
      ...
      <div className="overflow-hidden rounded-xl border border-border">
        <div className="grid grid-cols-3 bg-white/5 px-3 py-2 border-b border-border">
          ...
          <span className="text-[10px] font-mono text-mid text-right">{BEDS.find(b => b.key === selectedBed)?.label} {selectedCity}</span>
        </div>
        {[
          ...
          <div key={row.label} className={`grid grid-cols-3 px-3 py-2.5 border-b border-border last:border-0 ${i % 2 === 0 ? '' : 'bg-white/[0.02]'}`}>
            ...
            <span className="text-accent font-semibold text-center text-xs">
              {row.prop}{row.isPct && row.propRaw && row.cityRaw ? diff(row.propRaw, row.cityRaw, row.higherIsBetter) : null}
            </span>
      ...
      <p className="text-[10px] text-dim mt-3">
        {bedroomData ? `${BEDS.find(b => b.key === selectedBed)?.label} avg for ${selectedCity} · Zoopla 2026 · REalyse` : `City-wide avg for ${selectedCity} · ONS HPI · Zoopla April 2026`}
      </p>
    </div>
  )
}
