...
export async function GET(req: NextRequest) {
  ...
  if (uprn) {
    try {
      ...
      // Tier 1: LR postcode comparables (actual local sold prices, nationwide)
      // Tier 2: Homedata price_trends (outcode district average)
      // Tier 3: City growth-rate / city avg by bedroom
...
// ── COMPARABLE VALUATION ENGINE ───────────────────────────────────────────────
// Primary:   Land Registry postcode comparables — actual local sold prices,
//            time-adjusted to present using city HPI. Works for all UK postcodes.
// Secondary: Homedata price_trends (outcode-level district averages)
// Tertiary:  City growth-rate from last sold price / city avg by bedroom
...
async function calcComparableValuation(
  ...
  // Feature adjustment multiplier — applied consistently across all methods
  let featureAdj = 1.0
  ...
  // ── FALLBACK: growth-rate or city-avg ────────────────────────────────────────
  const fallback = (): ValuationResult => {
    ...
    // Outer London postcodes are 60–70% of the all-London average.
    // EN (Enfield), RM (Romford), DA (Dartford), IG (Ilford) ≈ 65%
  ...
  // Actual local sold prices in the same postcode, last 24 months.
  // EPC floor areas used per comparable where available → actual £/sqm, not estimated.
  // This mirrors Zoopla/Rightmove AVM methodology and works nationwide via LR open data.
  if (lrComps.length >= 2) {
    ...
    // For each comp: time-adjust price to present, get actual or typical floor area, compute £/sqm
    const compPsqm = workingComps.map(c => {
      ...
      // Use actual EPC floor area for this comparable if available, else typical for its type
      const compArea  = epcData.get(c.address.trim().split(/[\s,]/)[0].toLowerCase()) || getTypicalFloorArea(c.type)
      return adjPrice / compArea
    }).filter(v => v > 0 && v < 30000).sort((a, b) => a - b) // sanity: £0–30k/sqm

    if (compPsqm.length >= 2) {
      // Median £/sqm (robust to outliers — one distress sale or mansion doesn't skew)
      ...
      // Confidence increases with more comps, same-type match, actual EPC areas, and known subject area
      const confidence = Math.min(88,
        50 + compPsqm.length * 4 +
        (sameType.length >= 2 ? 5 : 0) +
        (subjectArea > 0 ? 8 : 0) +
        Math.min(epcHits * 3, 12) // up to +12 for EPC-backed comps
      )
  ...
  // ── TIER 2: HOMEDATA PRICE TRENDS (outcode-level district average) ────────────
  if (outcode) {
    try {
...
// ── EPC OPEN DATA ─────────────────────────────────────────────────────────────
// Source: epc.opendatacommunities.org (free — requires EPC_API_KEY + EPC_API_EMAIL)
// Single API call per property lookup returns:
//   floorAreas — Map<houseToken, sqm> for all postcode properties (used by valuation engine)
//   subjectEpc — normalised EPC record for the subject property (fills in missing Homedata EPC)
interface EpcResult {
...
async function fetchEpcData(
  ...
  try {
    ...
    // Subject's house number/name for matching
    ...
    for (const row of rows) {
      ...
      // Match subject property — prefer most recent EPC if multiple exist
...
// ── LAND REGISTRY DATA (history + postcode comparables) ──────────────────────
// Fetches all addresses in the postcode in parallel.
// Subject address transactions → history (all time)
// Other addresses transactions → comparables (last 24 months only)
...
async function fetchLrData(
  ...
  try {
    ...
    // Parallel fetch — all addresses simultaneously
