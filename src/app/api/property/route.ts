...
export async function GET(req: NextRequest) {
  ...
  if (uprn) {
    try {
      ...
      // Pass cityName into propRecord so bedroom price-banding fallback can use it
      const propRecordWithCity = { ...propRecord, _cityName: cityName }
...
// ── BEDROOM FALLBACK HELPERS ──────────────────────────────────────────────────
// These run only when existing primary and floor-area inference both fail.
...
function inferBedsFromHabitableRooms(
  ...
  let beds: number
  if (t === 'flat') {
    // flats: habitable rooms = bedrooms + 1 living room; studio allowed at 0
    beds = Math.max(0, habitableRooms - 1)
  } else {
    // houses: 5+ rooms typically have 2 reception rooms; smaller have 1
...
function enrichAttributes(
  ...
  if (beds === null) {
    // E. All fallbacks exhausted
    console.log('[beds] All fallbacks exhausted')
    bedroomsLabel      = 'Not recorded'
    bedroomsConfidence = 0
    bedroomsInferred   = false
  }
